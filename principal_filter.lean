...
theorem eventually_inf (P : X → Prop) {F F' : filter X} :
  eventually P (inf F F') ↔ (∃ Q R, eventually Q F ∧ eventually R F' ∧ (P ⊇ Q ∩ R)) :=
iff.intro
  (suppose eventually P (inf F F'),
    obtain Q HQ, from this,
    obtain R [(HR) (HP : P ⊇ Q ∩ R)], from and.elim_right HQ,
    have eventually Q F ∧ eventually R F' ∧ (P ⊇ Q ∩ R), from
      and.intro (and.elim_left HQ) (and.intro HR (λ x H, HP H)),
    show _, from exists.intro Q (exists.intro R this))
  (suppose ∃ Q R, eventually Q F ∧ eventually R F' ∧ (P ⊇ Q ∩ R), 
    obtain Q R [(HQ : eventually Q F) (HR : eventually R F') (HPQ : P ⊇ Q ∩ R )], from this,
    show _, from exists.intro Q (and.intro HQ (exists.intro R (and.intro HR HPQ))))

theorem eventually_Sup {s : set (filter X)} (P : set X):
...
theorem eventually_principal {s P : set X} : eventually P (principal s) ↔ (s ⊆ P) :=
iff.intro
...
theorem eventually_le_iff {A B : set X} : principal A ≼ principal B ↔ A ⊆ B := 
iff.intro
  (assume H : principal A ≼ principal B, 
    show _, from (iff.elim_left eventually_principal) (H !set_in_principal_set))
  (assume H : A ⊆ B,
    take s, suppose eventually s (principal B),
    have A ⊆ s, from subset.trans H ((iff.elim_left eventually_principal) this),
    show eventually s (principal A), from (iff.elim_right eventually_principal) this)

theorem sup_principal {A B : set X} : 
  sup (principal A) (principal B) = principal (A ∪ B) := 
have sets (sup (principal A) (principal B)) = sets (principal (A ∪ B)), from ext(take x, iff.intro
  (suppose H : eventually x (filter.sup (principal A) (principal B)),
    have A ⊆ x, from (iff.elim_left eventually_principal) (and.elim_left H),
    have B ⊆ x, from (iff.elim_left eventually_principal) (and.elim_right H),
    have A ∪ B ⊆ x, from λ y HAB, or.elim HAB (λ H, `A ⊆ x` H) (λ H, `B ⊆ x` H),
    show _, from (iff.elim_right eventually_principal) this)
  (suppose x ∈ principal (A ∪ B), 
    have A ∪ B ⊆ x, from (iff.elim_left eventually_principal) this,
    have A ⊆ x, from subset.trans (λ x H, or.inl H) this,
    have B ⊆ x, from subset.trans (λ x H, or.inr H) `A ∪ B ⊆ x`,
    show _, from and.intro
      ((iff.elim_right eventually_principal) `A ⊆ x`)
      ((iff.elim_right eventually_principal) this))),
filter.eq this

theorem inf_principal {A B : set X} : 
  inf (principal A) (principal B) = principal (A ∩ B) := 
have sets (inf (principal A) (principal B)) = sets (principal (A ∩ B)), from ext(take x, iff.intro
  (suppose H : x ∈ filter.inf (principal A) (principal B), 
    obtain Q HQ, from H,
    obtain R [(HR) (HP : x ⊇ Q ∩ R)], from and.elim_right HQ,
    have Q ⊇ A, from (iff.elim_left eventually_principal) (and.elim_left HQ),
    have R ⊇ B, from (iff.elim_left eventually_principal) HR,
    have x ⊇ A ∩ B, from λ y HAB,
      have y ∈ R, from `R ⊇ B` (and.elim_right HAB),
      have y ∈ Q, from `Q ⊇ A` (and.elim_left HAB),
      show y ∈ x, from HP (and.intro this `y ∈ R`),
    show _, from (iff.elim_right eventually_principal) this)
  (suppose x ∈ principal (A ∩ B), 
    have x ⊇ A ∩ B, from (iff.elim_left eventually_principal) this,
    have B ∈ principal B, from set_in_principal_set,
    have ∃₀ t ∈ principal B, x ⊇ A ∩ t, from exists.intro B (and.intro this `x ⊇ A ∩ B`),
    show _, from exists.intro A (and.intro set_in_principal_set this))),
filter.eq this
